...
/// Process a text into scalar engine (u, s).
fn process(text: &str, seed: u64) -> ScalarEngine {
    let mut eng = ScalarEngine::new();
    eng.init_u(seed);
    let s = build_source_f32(text);
    eng.set_source(&s);
...
fn fuse_energy(eq: &ScalarEngine, ed: &ScalarEngine) -> f32 {
    let mut fused = ScalarEngine::new();
    for k in 0..N {
        fused.u[k] = 0.5 * (eq.u[k] + ed.u[k]);
        fused.s[k] = eq.s[k] + ed.s[k];
    }
    let mx = (0..N).map(|k| fused.s[k].abs()).fold(0.0f32, f32::max);
    if mx > 0.0 { let inv = 1.0/mx; for k in 0..N { fused.s[k] *= inv; } }
