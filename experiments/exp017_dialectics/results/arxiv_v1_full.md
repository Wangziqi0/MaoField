...
**Mode-tag key for §2.3** (epistemic status of each claim, per the project's mode-tag discipline):
- `[Theorem]`: rigorous proof provided in text or cited
- `[Proposition]`: argument substantively complete; minor formal work remains (e.g., functor-law verification, explicit hom-set bijection)
- `[Conjecture]`: structurally supported by examples or partial computation, not proven
- `[Sketch]`: argument outline given, technical details deferred to follow-up
- `[Definition, symbolic first pass]`: notation or construct introduced for subsequent discussion; full rigorization pending

Each §2.3.x subsection opens by tagging its principal claims with the corresponding label. This is a direct response to the framework's own methodological commitment (§2.1.3 practice-as-criterion) and to the risk that a categorical formalization could otherwise be read as more settled than it is.

---

## 2.3.1 From Adjunction to Monad: The Closure of Dialectical Operation

**Principal claims** (tagged):
- Adjunction-level description of propose-return cycle via `(η, ε)` and triangle identities: `[Proposition]` (standard Lawvere 1969 material, adapted to our setting)
- The composition `T := G∘F` is **a monad** with multiplication `μ := G·ε·F`: `[Conjecture]` in the deterministic regime (see §2.3.3 for the structural obstruction and the stochastic-extension conjecture that would resolve it); `[Proposition]` as an endofunctor of `Meas` (§2.3.2)
- T-algebra as the categorical form of synthesis (Aufhebung): `[Definition, symbolic first pass]` — the definition is standard category theory, but its **identification** with the Hegelian/Maoist notion of synthesis is an interpretive claim
...
**Principal claims** (tagged):
- `T_sparse, T_dense : Meas → Meas` are endofunctors under the extension-by-identity scheme: `[Proposition]` (functor-law verification — identity preservation and composition — omitted for brevity; standard)
- `T_sparse` and `T_dense` correspond to **distinct** endofunctors (not just different points of a common endofunctor): `[Proposition]` supported by the §4.6 empirical phase/amplitude observations; a fully rigorous functorial distinction (explicit Z_n-equivariant category targets) is reserved for future work (footnote [^functor-future])
- The source-field functor `F` **co-determines** the algebra structure (materialist thesis): `[Proposition]` — the empirical evidence (phase-degenerate sparse vs amplitude-stratified dense) is sufficient to assert the co-determination; a stronger "categorical independence" statement (that the two endofunctors are non-isomorphic in some natural sense) remains `[Conjecture]`
...
**Principal claims** (tagged):
- Deterministic gradient flow is an `R⁺`-monoid action / F-coalgebra on field space, **not a monad**: `[Proposition]` (the argument — absence of canonical `μ: F∘F → F` satisfying coherence axioms in deterministic regime — is substantively complete but compressed; full proof is standard categorical material)
- The endofunctor `T_•` admits a **Giry-monad lift** `P ∘ T_•` inheriting `(η, μ)` from the underlying stochastic dynamics: `[Conjecture]` (explicitly labeled as such in text; rigorous formalization is an open problem)
- The **three-faces convergence** (categorical monad lift ≡ physical stochastic extension ≡ OP2 empirical co-design) onto a single stochastic structure: `[Conjecture]` — explicitly "suggested, not proven"; the rigorous demonstration of the convergence is itself part of the open problem
...
**Principal claims** (tagged):
- **Reachable subcategory** `T-Alg_T^{(η, p_0)} ⊊ T-Alg_T` in general: `[Proposition]` (Stage A1 at §4.7 provides the concrete empirical instance of strict inclusion — 0% occupancy of `|ψ|=2` basin under gradient-flow init)
- **Categorical gap** `Δ_{OP2} := |T-Alg_T| ⊖ |T-Alg_T^{(η, p_0)}|` as formal measure of OP2: `[Definition, symbolic first pass]` — cardinality complement is acknowledged as coarse; alternative quantifications (categorical entropy, persistent homology, Kan extension obstructions) are conjectured to agree in a common limit (footnote [^delta-op2-alt])
- **Kleisli-reachable path** definition: `[Sketch]` — requires the conjectured monad lift of §2.3.3; rigorous formulation of Kleisli composition `g ∘_K f := μ ∘ Tg ∘ f` is reserved for follow-up
- Well-definedness of `Δ_{OP2}` under proper-class / universe concerns: `[Proposition]` under restriction to `T-Alg_T ∩ U` for suitable Grothendieck universe `U` (standard set-theoretic hygiene)
