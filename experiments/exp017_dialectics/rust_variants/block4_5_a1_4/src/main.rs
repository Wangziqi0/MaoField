//! Block IV.5 Stage A1.4: SHIFTED multi-well (u∈{0,1,2}) Langevin dumper.
//! V_shifted = |psi|^2 * (|psi|^2 - 1)^2 * (|psi|^2 - 2)^2
//!   minima at u = |psi|^2 ∈ {0, 1, 2}
//!   saddles: inner u=0.2597 (V=0.4311), outer u=1.5403 (V=0.0950)
//!
//! Input: block3_input_100.json + embeddings_nfcorpus.json
//! Output: states.bin + meta.json
//!
//! Usage:
//!   block4_5_a1_4 <input.json> <emb.json> <states.bin> <meta.json> [dt] [steps] [clamp] [mode] [sigma]
...
fn evolve_emb(emb: &[f32], seed: u64, dt: f32, steps: usize, clamp_hi: f32, sigma: f32,
    ...
    -> (Vec<f32>, Vec<f32>, f32, f32, bool)
{
    ...
    if wall_lambda > 0.0 && sigma > 0.0 {
        eng.evolve_steps_walled_shifted_langevin(steps, dt, clamp_hi, sigma, seed,
                                                 wall_lambda, wall_k, wall_umax);
    } else if sigma > 0.0 {
        eng.evolve_steps_multiwell_shifted_langevin(steps, dt, clamp_hi, sigma, seed);
    } else {
        eng.evolve_steps_multiwell_shifted(steps, dt, clamp_hi);
    }
...
fn main() {
    ...
    let sigma: f32 = args.get(9).and_then(|s| s.parse().ok()).unwrap_or(0.0);
    let wall_lambda: f32 = args.get(10).and_then(|s| s.parse().ok()).unwrap_or(0.0);
    let wall_k: u32 = args.get(11).and_then(|s| s.parse().ok()).unwrap_or(4);
    let wall_umax: f32 = args.get(12).and_then(|s| s.parse().ok()).unwrap_or(2.5);
    ...
    eprintln!("DT={} steps={} clamp={} mode={} sigma={}", dt, steps, clamp_hi, mode, sigma);
    ...
    let results: Vec<(String, Vec<f32>, Vec<f32>, f32, f32, bool)> =
        docs_use.par_iter().enumerate().filter_map(|(i, d)| {
            ...
            let (a, b, umax, umean, nan) = evolve_emb(emb, seed, dt, steps, clamp_hi, sigma,
                                                        wall_lambda, wall_k, wall_umax);
    ...
    let meta = serde_json::json!({
        ...
        "evolve_steps": steps, "dt": dt, "clamp_hi": clamp_hi, "mode": mode, "sigma": sigma,
        "potential": if wall_lambda > 0.0 {
            format!("A1.4-walled: V_shifted + lambda*max(0,u-u_max)^k, lambda={} k={} u_max={}",
                    wall_lambda, wall_k, wall_umax)
        } else {
            "A1.4_shifted: V=|psi|^2*(|psi|^2-1)^2*(|psi|^2-2)^2, minima u∈{0,1,2}".to_string()
        },
        "barriers": "inner_saddle u=0.2597 V=0.4311; outer_saddle u=1.5403 V=0.0950",
        "wall": {"lambda": wall_lambda, "k": wall_k, "u_max": wall_umax},
        "dynamics": if wall_lambda > 0.0 && sigma > 0.0 { "walled_shifted_langevin" }
                    else if sigma > 0.0 { "langevin_shifted_multiwell" }
                    else { "gradient_flow_shifted_multiwell" },
