...
'use strict';
/**
 * MaoField Exp011: Residual Evolution Benchmark
 * token_new = token_original + β × evolution_residual
 *
 * Runs all residual_b*.sqlite variants + baseline.
 */
...
async function runOne(name, queryDb) {
  ...
  const v = new LawVexus(`/tmp/exp011_${name}`);
  ...
  return { ndcg_at_10: +avg.toFixed(4), n_queries: n, time_s: +sec };
}

async function main() {
  console.log('=== MaoField Exp011: Residual Evolution Benchmark ===\n');
  ...
  // Find residual variants
  const files = fs.readdirSync(EXP_DIR)
    .filter(f => f.startsWith('evolved_query_residual_') && f.endsWith('.sqlite'))
    .sort();
  ...
  // Summary
  console.log('\n' + '='.repeat(55));
  console.log('  Exp011 — Residual Evolution (β sweep)');
  ...
  for (const [n, r] of Object.entries(results).sort((a, b) => b[1].ndcg_at_10 - a[1].ndcg_at_10)) {
    ...
    console.log(`  ${n.padEnd(22)} ${r.ndcg_at_10.toFixed(4).padEnd(10)} ${d}`);
  }
  console.log('='.repeat(55));

  fs.writeFileSync(path.join(EXP_DIR, 'exp011_results.json'),
    JSON.stringify({ meta: { dataset: 'nfcorpus', date: '2026-04-09' }, results }, null, 2));
  console.log('\nSaved: exp011_results.json');
}
