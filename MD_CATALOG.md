...
| **RAG 重建 / 数据 digest ★** | `docs/infra/rag_rebuild_20260622/` + `scripts/rag_scan_maofield.py` + `scripts/rag_rebuild_maofield_36.sh` + `scripts/rag_rebuild_node22_runner.sh`; D623 report(7) refresh 后扫描 7775 files, 默认索引 311 active md / 8603 chunks + `maofield_data_digest_20260622.md`; node22 vector rebuild/refresh 记录见 `NODE22_VECTOR_REBUILD_20260622.md` 与 `NODE22_VECTOR_REFRESH_REPORT7_20260623.md`; 原始数据不直接入向量 |
