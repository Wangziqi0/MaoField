...
| **GPT deep-research 报告 ★** | `docs/infra/gpt_deep_research/` = GPT/PRO claim-source reports + adoption notes; report(3)=mean-null vector-field / LOSO-rank kill-test proposed audit; report(4)=external math-framework gate audit, repo facts blocked for auditor; report(6)=q4 strict bundle audit, full 50-checkpoint panel **not approved**, only implementation-review eligible after provenance + negative-smoke + fold-local analysis fixes; 均需回查 code/log/verdict/JSON |
| **RAG 重建 / 数据 digest ★** | `docs/infra/rag_rebuild_20260622/` + `scripts/rag_scan_maofield.py` + `scripts/rag_rebuild_maofield_36.sh` + `scripts/rag_rebuild_node22_runner.sh`; post-q4 扫描 7773 files / 475 md / 2112 structured-text data, 默认索引 309 active md / 8562 chunks + `maofield_data_digest_20260622.md`; node22 vector rebuild 记录见 `NODE22_VECTOR_REBUILD_20260622.md`; 原始数据不直接入向量 |
| **当前实验收敛 + 数学转向 gate ★** | `docs/infra/EXPERIMENT_CONVERGENCE_AND_MATH_TURN_20260622.md` + `docs/infra/math_turn_20260622/MATH_TURN_LOSO_AUDIT_VERDICT_20260622.md` + `docs/infra/math_turn_20260622/PANEL_PRIMARY_ARTIFACT_{COORDINATION,PREREG_DRAFT,RUNBOOK,SMOKE,NEGATIVE_SMOKE,MULTI_SMOKE}_20260622.md` + `docs/infra/math_turn_20260622/panel_schema_freq_q4_audit_targets_20260622.json` + `scripts/{math_turn_loso_audit.py,build_panel_schema_20260622.py,q4_panel_negative_smoke_20260622.py}` + `experiments/exp020_metric_stress_test/scripts/highorder_raw_logprob_panel.py` + `experiments/exp020_metric_stress_test/panel_primary_20260622/`; zero-GPU aggregate verdict=`insufficient_artifact`; q4 schema/runbook locked, one-checkpoint + negative + multi-checkpoint smoke pass, no full panel; report(6) strict audit says **do not approve full 50-checkpoint panel yet**; next allowed work=full-panel mode implementation + separate fold-local q4 analysis path review, 不开训练 |
