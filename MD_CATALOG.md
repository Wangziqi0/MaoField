...
> **2026-06-24 report(22) 主线收束补丁**: 新增 `docs/infra/gpt_deep_research/deep_research_quotient_residual_mainline_debranded_program_20260624.md` 与 `QUOTIENT_RESIDUAL_MAINLINE_ADOPTION_NOTE_20260624.md`。采纳为 report(17)/(19)/(21) 的主线合成: report(17)=问题化, report(19)=母对象化, report(21)=形式化/可击杀化, report(22)=negative-centered measurement-audit / no-go framework。当前 MaoField Mode B 仍 `insufficient_artifact`; smoke 仍 `smoke_conjecture_only`; 不运行 full panel / 不训练 / 不授权 new loss；默认 RAG 已刷新到 342 active md / 9027 chunks。
> **2026-06-24 null_tests contract 补丁**: 新增 `docs/infra/math_turn_20260622/NULL_TESTS_CONTRACT_20260624.md` 并收紧 `scripts/q4_hypercube_interaction_prereg_analysis.py`。future-only checker 不再接受 `null_tests` 八个名字的空壳存在性；每个 block 必须有 contract version、provenance、source-only weights、metrics、thresholds、布尔 `pass` 与 `kill_verdict_if_fail`。rank-shadow design-review floor 收紧为 `sigma2/sigma1 >= 0.25`。仍无 full panel / no training / no new result。
