...
**任何 forward-dated 命名 (file 命名 的 date 字段 > 实际 mtime) = D-1 纪律 1 占位符禁令 + 纪律 5 差异未记录 双重 violation**, 必须立即 retract + 校正 + 加 disclaimer (类 paper v8 head line 1-25 的 D17 校正模式)。
...
## D-3 反映论 work mode + 三机协作架构

原 line 224-598 (D-3 辩证唯物主义 260 行 + 三机协作架构 115 行) 已拆出到独立 docs:

- **D-3 反映论 work mode**: `docs/D-3-dialectical-reflection.md`
  含 D-3.1 标准次序 / D-3.2 6 binary corrected form / D-3.3-3.15 sub-sections + D21 reflexive cascade record + 4 path methodological + paradigm-shift candidate

- **三机协作架构**: `docs/three-machine-architecture.md`
  含三机角色 / 数据存放策略 / 备份策略 / Git+ssh 双保底 / 紧急 fallback / 一凡 self-action 列表

(关卡 1-3 反题三方决 + 哲学判读 涉及 D-3 时打开 `docs/D-3-*.md`; 三机配置 / 备份 / fallback 涉及时打开 `docs/three-machine-*.md`.)
