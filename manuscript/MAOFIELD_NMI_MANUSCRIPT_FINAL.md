...
Scientific history in an AI agent is not merely a current answer or stored fact, but a practice-grounded relation linking a rejected route, decisive test, evidence, scope and reopening condition. We preregistered a clean-successor causal protocol that randomized such histories while matching a prespecified evaluator-visible current-content map, then measured future tool use, executable tests and terminal world states. Phase 23 yielded opposed statistical extremes—exact treatment invariance and a near-saturated action–structure contrast—each reproduced in sealed held-out worlds; independent calibration showed that neither identified its apparent conclusion. A disjoint GLM-5.3-Flash branch passed ordinary behavioural controls yet established neither history inheritance, a bounded null nor structural dissociation. The combined result is an identification architecture: replication stabilizes a statistic, not its scientific meaning, and operational competence need not make an evaluator scientifically separating. Evaluator-visible current equivalence therefore does not establish future intervention equivalence without claim-specific separation.
...
The release package is designed to include the Master V3 authority chain, preregistration, an executable claim graph linking P0–P6 to treatments, estimands, figures and claim ceilings, world generators, renderers, model/provider request–response custody where redistribution is permitted, assignments after unblinding, both objective scorers, validators, analysis code, figure source data, typed failure ledgers, fail-stop lineages and a one-command deterministic reanalysis path. Provider secrets, API credentials, plaintext encryption keys and restricted hidden-answer material will not be released. Large-language-model tools assisted with evidence organization, code review, language editing and preliminary layout ideation. The author retained sole responsibility for scientific judgement, statistical decisions, claims and final text, and verified the reported calculations and source bindings. All scientific figures were generated deterministically from source data and manually checked; no generative-AI artwork was used.

## Data availability

The source data for all figures, result matrices and content-addressed manifests accompany the submission. A complete frozen archive will be deposited in Zenodo; DOI: **AUTHOR_INPUT_NEEDED**. Provider request–response material will be released to the maximum extent permitted by provider terms. Restricted records will be supplied to editors and reviewers through a persistent controlled-access archive, together with hashes, data dictionaries and derived sufficient statistics needed to reproduce every reported analysis; access location: **AUTHOR_INPUT_NEEDED**. API keys, plaintext encryption keys and restricted hidden-answer material will not be released.

## Code availability

All code required to reproduce the reported statistics, tables and figures without further model or provider calls is included in the reviewer release and will be made public in a versioned repository; repository URL: **AUTHOR_INPUT_NEEDED**. The release includes executable world generators where redistribution is permitted, scorer implementations, validators, statistical analysis, figure-generation scripts, environment manifests, deterministic smoke tests and SHA-256 ledgers. The tagged release will be archived in Zenodo; software DOI: **AUTHOR_INPUT_NEEDED**. A provider-dependent full rerun is documented separately and is not required to reproduce the reported analyses.
